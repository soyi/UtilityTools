...
const static CGFloat kBaseAlertViewDefaultButtonHeight       = 44;
const static CGFloat kBaseAlertViewDefaultButtonSpacerHeight = 0.5;
const static CGFloat kBaseAlertViewCornerRadius              = 12;
...
{
    if (delegate != NULL) {
        [delegate basedialogButtonTouchUpInside:self clickedButtonAtIndex:[sender tag]];
    }
...
- (void)basedialogButtonTouchUpInside: (BaseAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    [self close];
}
...
- (UIView *)createContainerView
{
    ...
    CGFloat cornerRadius = kBaseAlertViewCornerRadius;
...
- (void)addButtonsToView: (UIView *)container
{
    ...
    for (int i=0; i<[buttonTitles count]; i++) {
        ...
        [closeButton addTarget:self action:@selector(basedialogButtonTouchUpInside:) forControlEvents:UIControlEventTouchUpInside];
        ...
        [closeButton.layer setCornerRadius:kBaseAlertViewCornerRadius];
        
        [container addSubview:closeButton];
        
        if (i < [buttonTitles count]-1) {
            UIView *lineView = [[UIView alloc]initWithFrame:CGRectMake((i+1) * buttonWidth, container.bounds.size.height - buttonHeight, 0.5, buttonHeight)];
            lineView.backgroundColor = [UIColor colorWithRed:198.0/255.0 green:198.0/255.0 blue:198.0/255.0 alpha:1.0f];
...
- (CGSize)countScreenSize
{
    if (buttonTitles!=NULL && [buttonTitles count] > 0) {
        buttonHeight       = kBaseAlertViewDefaultButtonHeight;
        buttonSpacerHeight = kBaseAlertViewDefaultButtonSpacerHeight;
    } else {
        buttonHeight = 0;
